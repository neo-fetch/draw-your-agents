# Pytest dry-run (no API key needed) — generated by graphical-agents from the Graph IR (support_router_workflow).
...
def test_graph_builds() -> None:
    assert "process_message" in graph.get_graph().nodes
